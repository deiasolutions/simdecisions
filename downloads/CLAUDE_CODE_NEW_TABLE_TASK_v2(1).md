...
class ContentEntity(Base):
    ...
    def to_embedding_text(self) -> str:
        """
        Generate text for embedding - MATCH SURFACE only.
        
        INCLUDES: title, recognition (parent-voice phrases for Voyage matching)
        EXCLUDES: concept_summary (definition voice - wrong for semantic matching)
        EXCLUDES: guidance_do, guidance_dont, examples, red_flags, keywords
        
        The recognition field contains 25-30 messy, emotional, first-person 
        phrases that mirror how parents actually describe their situations.
        This is what Voyage embeds and matches against user input.
        
        concept_summary is injected into the prompt AFTER retrieval, not before.
        """
        parts = [self.title]
        if self.recognition:
            parts.append(self.recognition)
        return ". ".join(parts)
...
- Do NOT include concept_summary in embedding text (it's definition voice, not match surface)

---

## CRITICAL: Embedding Architecture

The embedding system uses a **two-surface architecture**:

**MATCH SURFACE (what Voyage embeds):**
- `title` + `recognition`
- Recognition contains 25-30 messy, emotional, first-person parent phrases
- Example: "she sent me another nasty email", "idk what to say back", "hes baiting me again"

**ANSWER SURFACE (what LLM sees after retrieval):**
- `title` + `concept_summary` + `guidance_do` + `guidance_dont` + `examples` + `red_flags`
- concept_summary is clean, clinical definition voice
- This is injected into prompt AFTER entity is retrieved

**Why this matters:**
- Parents say: "she sent me a 10 page text full of accusations what do i even say"
- concept_summary says: "BIFF is a framework for responding to hostile messages..."
- These don't match semantically. Embedding concept_summary causes retrieval failures.
- Recognition field mirrors parent voice → high match probability
